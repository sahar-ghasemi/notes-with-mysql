...
class Connection
{
    ...
    {
        $statement = $this->pdo->prepare("INSERT INTO notes (title, description, create_date,color)
         VALUES (:title,:description,:date,:color)");
        $statement->bindValue('title', $note['title']);
        $statement->bindValue('description', $note['description']);
        $statement->bindValue('date', date('Y-m-d H:i:s'));
        $statement->bindValue('color', $note['color']);
